#ifndef _QLOG_FILE_APPENDER_HEADER_
#define _QLOG_FILE_APPENDER_HEADER_
...
public:
    LogFile(int fd) : fd_(fd) {}
    ~LogFile() { if (fd_ >= 0) close(fd_); }
...
public:
    ...
    ~DynamicBuffer() { free(buf_); }
    char* ptr() const { return buf_; }
...
public:
    LogBuffer(size_t max): max_(max), logs_(0), index_(0) {}
    virtual ~LogBuffer() {}
    std::ostream& stream() { return stream_; }
    bool ShouldFlush() const;
    void AddLogCount() { logs_++; }
    size_t GetLogCount() const { return logs_; }
    void setfile(size_t log_index, const LogFilePtr& logfile) {index_ = log_index; file_ = logfile;}
    size_t index() { return index_; }
    LogFilePtr& file() { return file_; }
...
protected:
    ...
    void closeFiles();
    bool FlushBuffer(const boost::shared_ptr<LogBuffer>& buffer, bool force,
            bool unlock);
private:
...
class LOG4CPLUS_EXPORT RollingFileAppender : public qlog::FileAppender {
public:
...
class LOG4CPLUS_EXPORT DailyRollingFileAppender : public qlog::FileAppender {
public:
